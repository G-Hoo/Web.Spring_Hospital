...
<script>
 $(function() {
  ...
  <script>
  var app = app || {};  //namespace pattern , 다른 라이브러리에 app이라는 것이 있으면 초기화시켜버려라
  
  app.context=(function(){
     
  })(); 
  
  app.component=(function(){  //즉시 실행과 동시에 declaration을 하지 않는다. // JSON 리턴하는것이 Clouser
     return{
        body : $('#body'),
        wrapper : $('#wrapper'),
        div : function(id){
  ...
  app.complex={
         createButton:function(type){this._name=_name;},
          setAge:function(age){this._age=_age;},
          setGender:function(gender){this._gender=_gender;},
  ...
  */
  ...
  encap.on('click',function(){
     ...
     var tableRight=$('#tableRight');
     inputText.attr('placeholder','이름 나이 성별 직업 (공백으로 구분)').appendTo(tableRight);
     aButton.html('스펙보기').appendTo(tableRight).css('margin','10px auto').on('click',function(){
          var inputVal=$('#inputText').val();
          var arr=inputVal.split(' ');
          console.log('인캡클릭'+inputVal);
