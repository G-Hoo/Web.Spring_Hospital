...
<nav id="pagination">
	...
	<script>
		$(function(){
			...
			var boardTotal=$('#boardTotal');
			articleList.addClass('table_default').addClass('margin_center')
			.css('width','500px').css('position','relative').css('top','200px');
			articleList.find('tr:nth-child(1)').find('th:nth-child')
			.attr('colspan','5').css('text-align','right');
		});
		$(function(){
			var pagination=$('#pagination');
			pagination.css('width','500px').css('height','50px').css('margin','0 auto').css('background','gray').css('margin-top','205px');
			pagination.find('ul:nth-child(1)').addClass('gnb').css('width','100%');
		})
	</script>
